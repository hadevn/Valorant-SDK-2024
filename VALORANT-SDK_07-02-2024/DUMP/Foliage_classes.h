...
struct UFoliageStatistics : UBlueprintFunctionLibrary {
	...
	int32_t FoliageOverlappingSphereCount(struct UObject* WorldContextObject, struct UStaticMesh* StaticMesh, struct FVector CenterPosition, float Radius); // Function Foliage.FoliageStatistics.FoliageOverlappingSphereCount // (Final|Native|Static|Public|HasDefaults|BlueprintCallable) // @ game+0x5307330
	int32_t FoliageOverlappingBoxCount(struct UObject* WorldContextObject, struct UStaticMesh* StaticMesh, struct FBox Box); // Function Foliage.FoliageStatistics.FoliageOverlappingBoxCount // (Final|Native|Static|Public|HasDefaults|BlueprintCallable) // @ game+0x53071f0
};
...
struct AInteractiveFoliageActor : AStaticMeshActor {
	...
	struct FVector TouchingActorEntryPosition; // 0x3f0(0x0c)
	struct FVector FoliageVelocity; // 0x3fc(0x0c)
	struct FVector FoliageForce; // 0x408(0x0c)
	struct FVector FoliagePosition; // 0x414(0x0c)
	float FoliageDamageImpulseScale; // 0x420(0x04)
	float FoliageTouchImpulseScale; // 0x424(0x04)
	float FoliageStiffness; // 0x428(0x04)
	float FoliageStiffnessQuadratic; // 0x42c(0x04)
	float FoliageDamping; // 0x430(0x04)
	float MaxDamageImpulse; // 0x434(0x04)
	float MaxTouchImpulse; // 0x438(0x04)
	float MaxForce; // 0x43c(0x04)
	float Mass; // 0x440(0x04)
	char pad_444[0x4]; // 0x444(0x04)

	void CapsuleTouched(struct UPrimitiveComponent* OverlappedComp, struct AActor* Other, struct UPrimitiveComponent* OtherComp, int32_t OtherBodyIndex, bool bFromSweep, struct FHitResult& OverlapInfo); // Function Foliage.InteractiveFoliageActor.CapsuleTouched // (Final|Native|Protected|HasOutParms) // @ game+0x5306fd0
};
...
struct AProceduralFoliageBlockingVolume : AVolume {
	struct AProceduralFoliageVolume* ProceduralFoliageVolume; // 0x410(0x08)
};
...
struct UProceduralFoliageSpawner : UObject {
	...
	void Simulate(int32_t NumSteps); // Function Foliage.ProceduralFoliageSpawner.Simulate // (Final|Native|Public|BlueprintCallable) // @ game+0x5307cb0
};
...
struct AProceduralFoliageVolume : AVolume {
	struct UProceduralFoliageComponent* ProceduralComponent; // 0x410(0x08)
};
