...
struct FRCExposedPropertyDescription {
	...
	char pad_C[0x4]; // 0x0c(0x04)
	struct FString ID; // 0x10(0x10)
	struct FRCPropertyDescription UnderlyingProperty; // 0x20(0xa0)
	struct TMap<struct FName, struct FString> MetaData; // 0xc0(0x50)
	struct TArray<struct FRCObjectDescription> OwnerObjects; // 0x110(0x10)
};
...
struct FRCRequestWrapper : FRCRequest {
	...
	struct FName Verb; // 0x78(0x0c)
	int32_t RequestID; // 0x84(0x04)
};
...
struct FRCPresetFieldsAddedEvent {
	...
	char pad_1C[0x4]; // 0x1c(0x04)
	struct FString PresetId; // 0x20(0x10)
	struct FRCPresetDescription Description; // 0x30(0x40)
};
